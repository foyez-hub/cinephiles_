...
class profileController extends Controller
{
    ...
    function allData()
    {
        ...
        if (count($watchlistMap) == 0) {
            $watchlistMap = '';
        }
        if (count($favoritesMap) == 0) {
            $favoritesMap = '';
        }
        if (count($recentMap) == 0) {
            $recentMap = '';
        }
    ...
    function addcolor()
    {
        ...
        $data = [];
        $data[0] = $myinfo->watchlistp;
        $data[1] = $myinfo->favoritesp;
        $data[2] = $myinfo->recentlyp;
        return response()->json($data);
    }

    function sharethismeme(Request $request)
    {
        $myemail = $request->input('email') ;
        $image=$request->input('imagememe') ;
       ...
       if($this->isthere($memes,$image)==false){
        $memes.=','.$image.',';
    ...
    function removememe(Request $request)
    {
